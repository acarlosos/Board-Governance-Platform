        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('status')
                    ...
                    ->badge()
                    ->formatStateUsing(fn ($state): string => __('signature-signers.status.'.((string) $state))),
            ...
            ->headerActions([
                CreateAction::make()
                    ...
                    ->using(function (array $data) use ($request) {
                        return app(PersistSignatureSignerAction::class)->create(auth()->user(), $request, $data);
                    }),
            ])
            ->actions([
                EditAction::make()
                    ...
                    ->using(fn (SignatureRequestSigner $record, array $data) => app(PersistSignatureSignerAction::class)->update(auth()->user(), $record, $data)),
            ])
            ->bulkActions([]);
    }
}
