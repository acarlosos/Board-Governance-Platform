...
| Rollback release | regressão funcional | Reverter symlink/release Forge; `bash scripts/rollback.sh` para caches + workers; `php artisan migrate:rollback` só se migration nova falhou e reversão for segura |
