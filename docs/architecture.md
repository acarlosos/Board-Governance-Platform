...
| `App\Services\Dashboard\Executive\ExecutiveDashboardReadService` | **Orquestrador (19A.5)** — **`read(User, DashboardMetricsPeriod)`** → snapshot. **KPI**: `KpiStripProvider::build` **fora** de `Cache::flexible`; **Hero+Operations**: `Cache::flexible` sobre chave `dashboard_snapshot:{snapshot_version}:{segmento}:{period}:shared:plain` (payload só arrays — evita `__PHP_Incomplete_Class` ao `unserialize`); **Priorities/Activity** sempre frescos. **`cacheSegment()==none`** → sem L2 (recalcula em memória). Chaves L1/L2 canónicas: `ExecutiveDashboardCacheKeys` (**19B.1**). | L2 apenas Hero/Operations; L1 KPI separado (`dashboard_metrics:v1`)
