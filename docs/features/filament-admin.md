...
| `App\Filament\Admin\Resources\Boards\BoardResource` | `Board` | `BoardPolicy` + `BelongsToTenant`. Para `board_member`, listagem restringe aos boards onde participa como membro ativo. Na listagem: **Membros** abre modal com `BoardMembersRelationManager` (quem pode `update` o board). |
...
| `App\Filament\Admin\Resources\Votes\VoteResource` | `Vote` | `VotePolicy` + `BelongsToTenant`. Para participante, listagem restringe às reuniões onde participa. Opções só em draft; respostas read-only e, em voto secreto, identidade só para super-admin. |
